...
class TestConstructorNavigation:
    ...
    def test_navigation_to_fillings(self, driver):
        ...
        assert WebDriverWait(driver, 3).until(expected_conditions.presence_of_element_located(TestLocators.constructor_fillings_button_locator))

    def test_navigation_to_buns(self, driver):
        driver.get(url_main_page)
        driver.find_element(*TestLocators.constructor_fillings_button_locator).click()
        driver.find_element(*TestLocators.constructor_buns_button_locator).click()
        assert WebDriverWait(driver, 3).until(expected_conditions.presence_of_element_located(TestLocators.constructor_buns_button_locator))
