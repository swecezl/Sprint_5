...
def test_navigation_to_user_account_from_main_page(account_auth):
    account_auth.find_element(*TestLocators.user_account_header_button_locator).click()
    assert WebDriverWait(account_auth, 3).until(expected_conditions.url_to_be('https://stellarburgers.nomoreparties.site/account/profile'))


def test_navigation_to_constructor_from_user_account_page(account_auth):
    account_auth.find_element(*TestLocators.user_account_header_button_locator).click()
...
def test_navigation_logo_click_from_user_account_page(account_auth):
    account_auth.find_element(*TestLocators.user_account_header_button_locator).click()
